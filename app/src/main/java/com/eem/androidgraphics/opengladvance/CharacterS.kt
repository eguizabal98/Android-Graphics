...
import com.eem.androidgraphics.opengl.MyRenderer.Companion.loadShader
...
class CharacterS {
    ...
    private fun createCurve(controlPtsRight: FloatArray, controlPtsLeft: FloatArray) {
        ...
        while (i < controlPtsRight.size) {
            centrex += controlPtsRight[i]
            centrey += controlPtsRight[i + 1]
            i += 2
        }
            ...
            while (t < 1.0) {
                x =
                    (1.0 - t).pow(3.0) * controlPtsRight[controlptindex] + controlPtsRight[controlptindex + 2] * 3 * t * (1 - t).pow(
                        2.0) + controlPtsRight[controlptindex + 4] * 3 * t * t * (1 - t) + controlPtsRight[controlptindex + 6] * t.pow(
                        3.0)
                y =
                    (1.0 - t).pow(3.0) * controlPtsRight[controlptindex + 1] + controlPtsRight[controlptindex + 3] * 3 * t * (1 - t).pow(
                        2.0) + controlPtsRight[controlptindex + 5] * 3 * t * t * (1 - t) + controlPtsRight[controlptindex + 7] * t.pow(
                        3.0)
                xl =
                    (1.0 - t).pow(3.0) * controlPtsLeft[controlptindex] + controlPtsLeft[controlptindex + 2] * 3 * t * (1 - t).pow(
                        2.0) + controlPtsLeft[controlptindex + 4] * 3 * t * t * (1 - t) + controlPtsLeft[controlptindex + 6] * t.pow(
                        3.0)
                yl =
                    (1.0 - t).pow(3.0) * controlPtsLeft[controlptindex + 1] + controlPtsLeft[controlptindex + 3] * 3 * t * (1 - t).pow(
                        2.0) + controlPtsLeft[controlptindex + 5] * 3 * t * t * (1 - t) + controlPtsLeft[controlptindex + 7] * t.pow(
                        3.0)
    ...
    companion object {
        ...
        var CharSVertex: FloatArray = floatArrayOf()
        var CharSIndex: IntArray = intArrayOf()
        var CharSColor: FloatArray = floatArrayOf()
    }
